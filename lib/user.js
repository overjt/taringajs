...
User = (function() {
    ...
    User.prototype.getFollowers = function(user_id,callback) {
        var userId = user_id || this.taringa.user_id;
        var self;
        self = this;
        return this.taringa.request('http://api.taringa.net/user/followers/view/'+userId,
            function(error, response, body) {
            var data;
            if (error || response.statusCode !== 200) {
    ...
    User.prototype.getFollowings = function(user_id,callback) {
        var userId = user_id || this.taringa.user_id;
        var self;
        self = this;
        return this.taringa.request('http://api.taringa.net/user/followings/view/'+userId,
            function(error, response, body) {
            var data;
            if (error || response.statusCode !== 200) {
