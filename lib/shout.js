        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/shout/add', {
            ...
            if (!error && response.statusCode === 200) {
                try {
                    ...
                    if (data.status === 0) {
                        return self.taringa.log('Shout::add -> ' + S(data.data).decodeHTMLEntities().stripTags().s);
                    }
                } catch (_error) {
                    e = _error;
        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/comments/add', {
            ...
            if (!error && response.statusCode === 200) {
                try {
                    data = JSON.parse(body);
                    if (data.status === 0) {
                        return self.taringa.log('Shout::add_comment -> ' + S(data.data).decodeHTMLEntities().stripTags().s);
                    }
                } catch (_error) {
                    error = _error;
        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/comments/add', {
            ...
            if (!error && response.statusCode === 200) {
                try {
                    data = JSON.parse(body);
                    if (data.status === 0) {
                        return self.taringa.log('Shout::add_reply_comment -> ' + S(data.data).decodeHTMLEntities().stripTags().s);
                    }
                } catch (_error) {
                    error = _error;
        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/shout/vote', {
            ...
            if (!error && response.statusCode === 200) {
                ...
                if (data.status === 0) {
                    ...
                    return self.taringa.log('Shout::like -> ' + tmp);
                }
            } else {
    ...
    Shout.prototype.fav = function(id, owner_id) {
        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/shout/favorite-add', {
            ...
            if (!error && response.statusCode === 200) {
                ...
                if (data.status === 0) {
                    tmp = S(data.data).decodeHTMLEntities().stripTags().s;
                    return self.taringa.log('Shout::fav -> ' + tmp);
                }
            } else {
    ...
    Shout.prototype.attach_link = function(link,callback) {
        ...
        return this.taringa.request.post('http://www.taringa.net/ajax/shout/attach', {
            ...
            if (!error && response.statusCode === 200) {
                ...
                if (data.status === 0) {
                    tmp = S(data.data).decodeHTMLEntities().stripTags().s;
                    return callback.call(this, 'Shout::attach_link -> ' + tmp);
                }
